...
def clone_scorm_resource(source_course_key, destination_course_key):
    source_scorm_blocks = modulestore().get_items(source_course_key, qualifiers={'category': 'scormxblock'})
    dest_scorm_blocks = modulestore().get_items(destination_course_key, qualifiers={'category': 'scormxblock'})
    using_s3 = settings.DJFS['type'] == 's3fs'
    for block in source_scorm_blocks:
        block_id = block.location.block_id
        source_file_path = block.fs.root_path if not using_s3 else u'/'
        for dest_block in dest_scorm_blocks:
            if dest_block.location.block_id == block_id:
                dest_file_path = dest_block.fs.root_path if not using_s3 else u'/'
                if using_s3:
                    copy_dir(block.fs, source_file_path, dest_block.fs, dest_file_path)
                else:
                    copy_tree(source_file_path, dest_file_path)
...
def rerun_course(source_course_key_string, destination_course_key_string, user_id, fields=None):
    ...
    try:
        ...
        # clone scorm resource files
        clone_scorm_resource(source_course_key, destination_course_key)
