...
def _save_xblock(user, xblock, data=None, children_strings=None, metadata=None, nullout=None,
    ...
    with store.bulk_operations(xblock.location.course_key):
        ...
        if publish == "discard_changes":
            ...
            log.info('User {user_id} discarded changes of {block_type}: {display_name}, block_id: {block_id}'.format(
                user_id=user.id, block_type=xblock.category, display_name=xblock.display_name.encode('utf-8'),
                block_id=xblock.location
            ))
            ...
            if metadata is not None:
                for metadata_key, value in metadata.items():
                    ...
                    if value is None:
                        ...
                        old_val = getattr(xblock, metadata_key, None)
                        new_val = value
                        if type(old_val) in (str, unicode):
                            old_val = old_val.encode('utf-8')
                        if type(new_val) in (str, unicode):
                            new_val = new_val.encode('utf-8')
                        if old_val != new_val:
                            log.info('User {user_id} edited {attr} from "{old_val}" to "{new_val}", '
                                     'block: {block_id}'.format(user_id=user.id,
                                                                attr=metadata_key,
                                                                old_val=old_val,
                                                                new_val=new_val,
                                                                block_id=xblock.location))
        ...
        if publish == 'make_public':
            ...
            log.info('User {user_id} published {block_type}: {display_name}, block_id: {block_id}'.format(
                user_id=user.id, block_type=xblock.category, display_name=xblock.display_name.encode('utf-8'),
                block_id=xblock.location
            ))
            clear_course_from_cache(xblock.location.course_key)
...
def _create_item(request):
    ...
    log.info('User {user_id} created {block_type}: {title}, block_id: {block_id}, course_id: {course_id}'.format(
        user_id=request.user.id,
        block_type=category,
        title=created_block.display_name.encode('utf-8'),
        block_id=created_block.location,
        course_id=created_block.location.course_key))
...
def _delete_item(usage_key, user):
    ...
    with store.bulk_operations(usage_key.course_key):
        ...
        log.info('User {user_id} deleted {block_tye}: {block_id}, course_id: {course_id}.'.format(
            user_id=user.id, block_tye=usage_key.category, block_id=usage_key, course_id=usage_key.course_key
        ))
