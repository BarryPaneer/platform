...
def _upload_asset(request, course_key):
    ...
    locked = getattr(content, 'locked', True)
    logging.info('User {user_id} added "{file_name}" to Files & Uploads, course_id: {course_id}'.format(
        user_id=request.user.id, file_name=upload_file.name, course_id=course_key
    ))
    return JsonResponse({
...
def _update_asset(request, course_key, asset_key):
    ...
    if request.method == 'DELETE':
        try:
            ...
            logging.info('User {user_id} deleted asset: {file_name}, asset_key: {asset_key}, '
                         'course_id: {course_id}'.format(user_id=request.user.id,
                                                         file_name=asset_key.name,
                                                         asset_key=asset_key,
