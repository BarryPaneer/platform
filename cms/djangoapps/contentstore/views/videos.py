...
def videos_handler(request, course_key_string, edx_video_id=None):
    ...
    if request.method == "GET":
        ...
        logging.info('User {user_id} deleted video: {video_id}, course_id: {course_id}'.format(
            user_id=request.user.id, video_id=edx_video_id, course_id=course_key_string
        ))
        return JsonResponse()
    else:
...
def videos_post(course, request):
    ...
    for req_file in req_files:
        ...
        logging.info('User {user_id} uploaded video: {video_name}, video_id: {video_id}, course_name: {course_name}, '
                     'course_id: {course_id}'.format(user_id=request.user.id,
                                                     video_name=file_name.encode('utf-8'),
