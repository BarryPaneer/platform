...
from milestones.models import Milestone, CourseMilestone
...
def course_listing(request):
    ...
    return render_to_response(u'index.html', {
        u'is_program_enabled': is_program_enabled,
        u'orgs': orgs,
...
def settings_update_handler(request, course_key_string):
    ...
    with modulestore().bulk_operations(course_key):
        if request.method == 'GET':
            ...
            try:
                engaged_by_LP = any(DraftPartialProgram.test_engaged_LP_by_courses_visibility(course_key_string).values())
                if engaged_by_LP:
                    return JsonResponseBadRequest({'error': _("You cannot delete this course because it is used in Learning Paths")})

                delete_course(course_key, ModuleStoreEnum.UserID.mgmt_command, remove_course_reports=True,
                              remove_course_search_index=True)
                log.info('User {} deleted course "{}", course_id: {}'.format(
                    request.user.id, course_module.display_name, course_key))
        ...
        # For every other possible method type submitted by the caller...
            ...
            if ProgramsApiConfig.is_enabled():
                used_by_non_private_Lp = DraftPartialProgram.test_engaged_LP_by_courses_visibility(course_key_string)['used_by_non_private']
                if 'catalog_visibility' in request.json and request.json['catalog_visibility'] == 'none' and used_by_non_private_Lp:
                    log.exception(text_type("You cannot change the visibility status of this course to \"Private\", because it belongs to a \"Full Public\" or \"Accessible by URL\" learning path(s)."))
                    return JsonResponseBadRequest({'error': _("You cannot change the visibility status of this course to \"Private\", because it belongs to a \"Full Public\" or \"Accessible by URL\" learning path(s).")})

            # Perform the normal update workflow for the CourseDetails model
            return JsonResponse(
                CourseDetails.update_from_json(course_key, request.json, request.user, request=request),
                encoder=CourseSettingsEncoder
            )
...
def settings_handler(request, course_key_string):
    ...
    with modulestore().bulk_operations(course_key):
        ...
        if 'text/html' in request.META.get('HTTP_ACCEPT', '') and request.method == 'GET':
            ...
            # check if the course is prerequisite of other courses
            milestones = Milestone.objects.filter(namespace=course_key_string).values_list('id', flat=True)
            course_milestones = CourseMilestone.objects.filter(
                active=True,
                milestone_id__in=milestones,
                milestone_relationship_type_id=2,
            ).values_list('course_id', flat=True)
            fulfilled_courses = []
            for course_id in course_milestones:
                try:
                    fulfilled_course = modulestore().get_course(CourseKey.from_string(course_id))
                    fulfilled_courses.append(
                        (fulfilled_course.display_name, reverse_course_url('settings_handler', course_id))
                    )
            ...
            settings_context = {
                ...
                'optionObject': {'both': _('full public'), 'about': _('accessible by URL'), 'none': _('private')},
            ...
            settings_context['used_by_private'] = False
            settings_context['used_by_non_private'] = False
            if ProgramsApiConfig.is_enabled():
                _filter = {'courses.course_runs.key': course_key_string}
                def _gen_program_info(_program):
                    return {'title': _program['title'], 'uuid': _program['uuid']}
                _linked_programs = [
                    _gen_program_info(_program.to_dict())
                    for _program in DraftPartialProgram.query(_filter, limit=4)
                ]
                if _linked_programs:
                    settings_context.update({'linked_programs': _linked_programs})
                settings_context.update(DraftPartialProgram.test_engaged_LP_by_courses_visibility(course_key_string))
    ...
    with modulestore().bulk_operations(course_key):
        ...
        if 'text/html' in request.META.get('HTTP_ACCEPT', '') and request.method == 'GET':
            ...
            return render_to_response('settings_advanced.html', {
                ...
                'anderspinks_boards_list': anderspinks_boards_list
            })
        elif 'application/json' in request.META.get('HTTP_ACCEPT', ''):
            if request.method == 'GET':
...
def textbooks_list_handler(request, course_key_string):
    ...
    with store.bulk_operations(course_key):
        ...
        if request.method == 'GET':
            ...
            log.info('User {user_id} created textbook: {title}, tab_id: {tab_id}, course_name: {course_name}, '
                     'course_id: {course_id}'.format(user_id=request.user.id,
                                                     title=textbook['tab_title'],
                                                     tab_id=textbook['id'],
...
def textbooks_detail_handler(request, course_key_string, textbook_id):
    ...
    with store.bulk_operations(course_key):
        ...
        if request.method == 'GET':
            ...
            store.update_item(course_module, request.user.id)
            log.info('User {user_id} deleted textbook: {title}, tab_id: {tab_id}, course_name: {course_name}, '
                     'course_id: {course_id}'.format(user_id=request.user.id,
                                                     title=textbook['tab_title'],
                                                     tab_id=textbook['id'],
