...
def get_component_templates(courselike, library=False):
    ...
    ordered_quiz_list = ["Blank Quiz", "Dropdown", "MCQ: single answer", "MCQ: multiple answers", "Text Input",
                         "Numerical Input", "Open Response Assessment", "Drag and Drop", "Image Mapped Input",
                         "Randomized Content Block", "Intermediate Certificate"]
    current_quiz_list = [i["display_name"] for i in component_templates[2]["templates"]]
    new_list = []
    for quiz in ordered_quiz_list:
        if _(quiz) in current_quiz_list:
            idx = current_quiz_list.index(_(quiz))
            new_list.append(component_templates[2]["templates"][idx])
    component_templates[2]["templates"] = new_list
