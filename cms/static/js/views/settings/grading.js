        ...
        // would love to move to a general superclass, but event hashes don't inherit in backbone :-(
            ...
            render: function() {
                ...
                gradeCollection.each(function (gradeModel) {
                    let assignment_threshold_message = gettext("The threshold of all assignments of this type, by default it is 100. Do not include the percent symbol.").replace("100", self.options.assignment_threshold)
                    HtmlUtils.append(gradelist, self.template({model: gradeModel, assignment_threshold_message}));
            ...
            addAssignmentType: function (e) {
                e.preventDefault();
                this.model.get('graders').push({threshold : this.options.assignment_threshold});
            },
            fieldToSelectorMap: {
            ...
            renderCutoffBar: function () {
                ...
                _.each(this.descendingCutoffs, function (cutoff, index) {
            ...
            startMoveClosure: function () {
                ...
                return function (event, ui) {
                    ...
                    var min = (barIndex < cachethis.descendingCutoffs.length ? cachethis.descendingCutoffs[barIndex].cutoff + 1 : 1);
                    // minus 2 b/c minus 1 is the element we're effecting. It's max is just shy of the next one above it
                    var max = (barIndex >= 2 ? cachethis.descendingCutoffs[barIndex - 2].cutoff - 0 : 100);
