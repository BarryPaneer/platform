...
NPM_INSTALLED_LIBRARIES = [
    ...
    'learningtribes-studio-frontend/',
...
def process_npm_assets():
    ...
    def copy_vendor_library_dir(library_dir, skip_if_missing=False):
        ...
        if os.path.exists(library_dir_path):
            vendor_dir_path = os.path.join(NPM_JS_VENDOR_DIRECTORY, library_dir)
            sh('/bin/cp -rf {library_path} {vendor_dir}'.format(
                library_path=library_dir_path,
                vendor_dir=vendor_dir_path,
            ))
        elif not skip_if_missing:
            raise Exception('Missing vendor directory {library_path}'.format(library_path=library_dir_path))
...
def update_assets(args):
    ...
    if args.settings=='aws':
        try:
            sh("curl --request POST 'https://devops-dashboard.triboolearning.com/api/downtimelog' --header 'Authorization: Token a5e3d87afc80a3848251919dc83303785a502063' --data-urlencode client_hostname=$(hostname) --data-urlencode 'downtime_log=start compile'")
        except:
            pass
    ...
    if args.settings=='aws':
        try:
            sh("curl --request POST 'https://devops-dashboard.triboolearning.com/api/downtimelog' --header 'Authorization: Token a5e3d87afc80a3848251919dc83303785a502063' --data-urlencode client_hostname=$(hostname) --data-urlencode 'downtime_log=end compile'")
        except:
            pass