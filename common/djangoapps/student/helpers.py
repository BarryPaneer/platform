...
def generate_activation_email_context(user, registration):
    ...
    return {
        ...
        'support_email': configuration_helpers.get_value('TECH_SUPPORT_EMAIL', settings.TECH_SUPPORT_EMAIL),
    }
        ...
        if has_html_certificates_enabled(course_overview):
            if course_overview.has_any_active_web_certificate:
                status_dict.update({
                    ...
                    'download_url': get_certificate_url(course_id=course_overview.id, uuid=cert_status['uuid'])
                })
            else:
