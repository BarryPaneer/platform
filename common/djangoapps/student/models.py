...
class UserProfile(models.Model):
    ...
    service_id = models.CharField(blank=True, max_length=255, db_index=True)
    ...
    lt_ilt_supervisor = models.EmailField(verbose_name='ILT Supervisor', blank=True, null=True, db_index=True)
...
def user_post_save_callback(sender, **kwargs):
    ...
    if 'is_active' in changed_fields or 'email' in changed_fields:
        if user.is_active:
            ...
            # ProgramEnrollmentAllowed is used when a user is allowed to enroll in a program.
            peas = ProgramEnrollmentAllowed.for_user(user).filter(auto_enroll=True)
            ...
            from lms.djangoapps.program_enrollments.api import write_program_enrollment, \
...
class CourseEnrollment(models.Model):
    ...
    enrolled_datetime = models.DateTimeField(null=True)
    completed = models.DateTimeField(default=None, null=True)
    gradebook_edit = models.DateTimeField(default=None, null=True)
    ...
    def update_enrollment(self, mode=None, is_active=None, skip_refund=False):
        ...
        if activation_changed or mode_changed:
            if is_active:   # Update `enrolled  datetime`
                self.enrolled_datetime = datetime.now(UTC)
            self.save()
...
class ProgramEnrollmentAllowed(DeletableByUserValue, models.Model):
    """
    Table of users (specified by email address strings) who are allowed to enroll in a specified program.
    The user may or may not (yet) exist.  Enrollment by users listed in this table is allowed
    even if the enrollment time window is past.  Once an enrollment from this list effectively happens,
    the object is marked with the student who enrolled, to prevent students from changing e-mails and
    enrolling many accounts through the same e-mail.
    """
    email = models.CharField(max_length=255, db_index=True)
    program_id = models.UUIDField(db_index=True, null=False)
    auto_enroll = models.BooleanField(default=0)
    user = models.ForeignKey(
        User,
        null=True,
    ...
    class Meta(object):
        unique_together = (('email', 'program_id'),)

    def __unicode__(self):
        return "[ProgramEnrollmentAllowed] %s: %s (%s)" % (self.email, self.program_id, self.created)

    @classmethod
    def for_user(cls, user):
        """
        Returns the ProgramEnrollmentAllowed objects that can effectively be used by a particular `user`.
        This includes the ones that match the user's e-mail and excludes those PEA which were already consumed
        by a different user.
        """
        return cls.objects.filter(email=user.email).filter(Q(user__isnull=True) | Q(user=user))
    ...
    def valid_for_user(self, user):
        """
        Returns True if the PEA is usable by the given user, or False if it was already consumed by another user.
        """
        return self.user is None or self.user == user

    @classmethod
    def may_enroll_and_unenrolled(cls, program_id):
        """
        Return QuerySet of students who are allowed to enroll in a program.

        Result excludes students who have already enrolled in the
        program.

        `program_id` identifies the program for which to compute the QuerySet.
        """
        from lms.djangoapps.program_enrollments.constants import ProgramEnrollmentStatuses
        from lms.djangoapps.program_enrollments.models import ProgramEnrollment

        enrolled = ProgramEnrollment.objects.filter(
            program_uuid__in=[program_id],
            status=ProgramEnrollmentStatuses.ENROLLED
        ).values_list(
            'user__email', flat=True
        )
        return ProgramEnrollmentAllowed.objects.filter(program_id=program_id).exclude(email__in=enrolled)
