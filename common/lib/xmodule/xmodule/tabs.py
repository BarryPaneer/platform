...
class CourseTabList(List):
    ...
    def iterate_displayable(course, user=None, inline_collections=True, include_hidden=False):
        ...
        """
        for tab in course.tabs:
            is_tab_staff_displayable = tab.type == 'static_tab' and tab.course_staff_only
            is_tab_not_hidden = not (user and tab.is_hidden)
            if tab.is_enabled(course, user=user) and ((include_hidden or is_tab_not_hidden) or is_tab_staff_displayable):
