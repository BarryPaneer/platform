...
(function() {
    ...
    this.Sequence = (function() {
        function Sequence(element) {
            ...
            this.selectNone = function(event) {
                return Sequence.prototype.selectNone.apply(self, [event]);
            };
            ...
            this.courseLink = $(".back-link").children().first().attr("href");
        }
        ...
        Sequence.prototype.enableButton = function(buttonClass, buttonAction) {
            if (buttonClass === ".sequence-nav-button.button-next") {
                this.$(buttonClass).css("display", "block");
                this.$(".sequence-nav-button.button-finish").css("display", "none");
        ...
        Sequence.prototype.disableButton = function(buttonClass) {
            if (buttonClass === ".sequence-nav-button.button-next") {
                this.$(buttonClass).css("display", "none");
                this.$(".sequence-nav-button.button-finish").css("display", "block");
        ...
        Sequence.prototype.toggleArrows = function() {
            ...
            if (isLastTab && this.nextUrl === "None") {
                this.updateButtonState(nextButtonClass, this.selectNone, isLastTab, this.nextUrl);
            } else {
        ...
        Sequence.prototype.selectNone = function(event) {
            this._change_sequential('finish', event);
        };
        ...
        Sequence.prototype._change_sequential = function(direction, event) {
            ...
            if (direction !== 'previous' && direction !== 'next') {
                event.preventDefault();
                window.location.href = this.courseLink;
                return;
            }
            event.preventDefault();
        ...
        Sequence.prototype.update_completion = function(position) {
            ...
            sequences.each(function (index, value) {
                if (value.dataset['id'] == usageKey) {
                    completionIndicator = $(this).find('.fa-shield, .fa-circle');
                }
            });
            if (completionIndicator.length) {
                $.postWithPrefix(completionUrl, {
                    usage_key: usageKey
                }, function(data) {
                    if (data.complete === true) {
                        completionIndicator.removeClass('fa-shield').removeClass('fa-circle');
                        if (data.graded === true) {
                            completionIndicator.addClass('fa-shield-check');
                        } else {
                            completionIndicator.addClass('fa-check-circle');
                        }
                    }
                });
            }
