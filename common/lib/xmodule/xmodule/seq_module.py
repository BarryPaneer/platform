        ...
        for item in display_items:
            ...
            if is_user_authenticated:
                if item.location.block_type == 'vertical':
                    ...
                    iteminfo['complete'] = is_vertical_completed(completion_service, item, user)
...
def is_vertical_completed(service, block, user):
    from course_blocks.api import get_course_blocks
    for child in block.children:
        if child.block_type == 'library_content':
            child_locations = get_course_blocks(user, child)
            completions = service.get_completions(child_locations)
            for child_location in child_locations:
                if completions[child_location] < 1.0:
                    return False
    return service.vertical_is_complete(block)
