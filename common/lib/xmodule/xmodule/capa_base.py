...
class CapaMixin(ScorableXBlockMixin, CapaFields):
    ...
    def _gettext(self, msg):
        """
        given a msg get the i18n translation 
        """
    ...
    def submit_button_submitting_name(self):
        ...
        """
        return self._gettext('Submitting')
    ...
    def handle_problem_html_error(self, err):
        ...
        if self.runtime.DEBUG:
            ...
            warning_msg = self._gettext("Warning: The problem has been reset to its initial state!")
    ...
    def get_demand_hint(self, hint_index):
        ...
        while counter <= hint_index:
            ...
            total_text = HTML(self._gettext('{previous_hints}{list_start_tag}{strong_text}{hint_text}</li>')).format(
                ...
                strong_text=HTML('<strong>{hint_number_prefix}</strong>').format(
                    ...
                    hint_number_prefix=Text(self._gettext("Hint ({hint_num} of {hints_count}): ")).format(
            ...
            try:
                ...
                if "<p>Explanation</p>" in answer_content:
                    answer_content = answer_content.replace(
                        "<p>Explanation</p>",
                        "<p>{}</p>".format(self._gettext("Explanation"))
                    )
                if self.runtime.replace_jump_to_id_urls:
    ...
    def submit_problem(self, data, override_time=False):
        ...
        if self.closed():
            ...
            raise NotFoundError(self._gettext("Problem is closed."))
        ...
        if self.done and self.rerandomize == RANDOMIZATION.ALWAYS:
            ...
            raise NotFoundError(self._gettext("Problem must be reset before it can be submitted again."))
        ...
        if self.lcp.is_queued():
            ...
            if (current_time - prev_submit_time).total_seconds() < waittime_between_requests:
                msg = self._gettext(u"You must wait at least {wait} seconds between submissions.").format(
        ...
        if self.last_submission_time is not None and self.submission_wait_seconds != 0:
            ...
            if seconds_since_submission < self.submission_wait_seconds:
                ...
                msg = self._gettext(u'You must wait at least {wait_secs} between submissions. {remaining_secs} remaining.').format(
    ...
    def save_problem(self, data):
        ...
        if self.closed() and not self.max_attempts == 0:
            ...
            return {
                ...
                'msg': self._gettext("Problem is closed."),
        ...
        if self.done and self.rerandomize == RANDOMIZATION.ALWAYS:
            ...
            return {
                'success': False,
                'msg': self._gettext("Problem needs to be reset prior to save.")
            }
        ...
        msg = self._gettext("Your answers have been saved.")
    ...
    def reset_problem(self, _data):
        ...
        if self.closed():
            ...
            return {
                ...
                # pylint: disable=line-too-long
                # Translators: 'closed' means the problem's due date has passed. You may no longer attempt to solve the problem.
                'msg': self._gettext("You cannot select Reset for a problem that is closed."),
                # pylint: enable=line-too-long
            }
        ...
        if not self.is_submitted():
            ...
            return {
                'success': False,
                'msg': self._gettext("You must submit an answer before you can select Reset."),
            }
    ...
    def rescore(self, only_if_higher=False):
        ...
        if not self.lcp.supports_rescoring():
            ...
            raise NotImplementedError(self._gettext("Problem's definition does not support rescoring."))
            # pylint: enable=line-too-long

        if not self.done:
            event_info['failure'] = 'unanswered'
            self.track_function_unmask('problem_rescore_fail', event_info)
            raise NotFoundError(self._gettext("Problem must be answered before it can be graded again."))
