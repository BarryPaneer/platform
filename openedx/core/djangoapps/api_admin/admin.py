...
class ApiAccessRequestAdmin(admin.ModelAdmin):
    ...
    def get_readonly_fields(self, request, obj=None):
        """Only approve api account could access field `status`"""
        readonly_fields = (
            'user', 'email_address', 'website', 'reason',
            'company_name', 'company_address', 'contacted'
        )
        if obj.user.username != 'api_user':
