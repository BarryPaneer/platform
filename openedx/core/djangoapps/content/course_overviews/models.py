...
class CourseOverview(TimeStampedModel):
    ...
    def dashboard_start_display(self):
        ...
        """
        if self.advertised_start:   # TextField(null=True)
            try:
                return datetime.strptime(self.advertised_start, '%m/%d/%Y')
            except:
                pass

        return self.start           # DateTimeField(null=True)
    ...
    @classmethod
    def get_all_courses(cls, orgs=None, filter_=None, teams_courses_user=None):
        """
        ...
        Arguments:
            ...
            teams_courses_only (boolean): Optional parameter that allows exposing
                courses which in user's course teams only.
        """
        ...
        if teams_courses_user is not None:
            # Get all courses IDs that user can access.
            #
            #     1. Platform Admin / Super Admin could access all courses.
            #     2. Other roles only access his courses of course teams.
            if not teams_courses_user.is_superuser and \
                    (
                        STUDIO_ADMIN_ACCESS_GROUP in [group.name for group in teams_courses_user.groups.all()] and
                        not teams_courses_user.is_staff
                    ):
                # Filter courses which only related with user's Course Team(s)
                course_overviews = course_overviews.filter(
                    id__in=Subquery(
                        CourseAccessRole.objects.filter(
                            user_id=teams_courses_user
                        ).values(
                            'course_id'
                        )
                    )
