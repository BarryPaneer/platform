...
from common.djangoapps.student.roles import studio_access_role
...
class StaticContentServer(object):
    ...
    def is_program_asset_request(self, request):
        """Serves program assets to end users.
        """
        return request.path.startswith(
            r'/{}:'.format(PartialProgram.PREFIX_PROGRAM_CARD_IMAGE)
        )
    ...
    def process_request(self, request):
        ...
        if self.is_asset_request(request):
            ...
            return response
        elif self.is_program_asset_request(request):
            asset_path = asset_path[1:] \
                if '/' == asset_path[0] else asset_path

            programs_cards_dfs = PartialProgram.get_program_card_dfs()
            card_image = programs_cards_dfs.get_data_by_key(asset_path)

            response = HttpResponse(card_image)
            response['Content-Length'] = card_image.length
            response['Accept-Ranges'] = 'bytes'
            response['Content-Type'] = card_image.content_type
    ...
    def is_user_authorized(self, request, content, location):
        ...
        is_staff_or_instructor = has_access(request.user, 'staff', location.course_key) or \
                                 has_access(request.user, 'instructor', location.course_key)
        if not studio_access_role(request.user) and not is_staff_or_instructor:
