...
def get_visible_courses(
        org=None, filter_=None, teams_courses_user=None, return_all_flag=True
):
    """
    ...
    Arguments:
        ...
        teams_courses_user (User): Optional parameter that allows exposing
    ...
    if org:
        ...
        if not current_site_orgs or org in current_site_orgs:
            courses = CourseOverview.get_all_courses(
                orgs=[org], filter_=filter_, teams_courses_user=teams_courses_user
            )
    elif current_site_orgs:
        # Only display courses that should be displayed on this site
        courses = CourseOverview.get_all_courses(
            orgs=current_site_orgs, filter_=filter_, teams_courses_user=teams_courses_user
        )
    else:
        courses = CourseOverview.get_all_courses(
            filter_=filter_, teams_courses_user=teams_courses_user
        )

    courses = sorted(courses, key=lambda course: course.number)

    # Filtering can stop here.
    if current_site_orgs or return_all_flag:
        return courses
