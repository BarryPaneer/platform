...
def certificate_downloadable_status(student, course_key):
    ...
    response_data = {
        ...
        'download_url': None,
        'uuid': None,
        'not_passing': True if current_status['status'] == CertificateStatuses.notpassing else False
    }
...
def cert_self_generation_enabled(course_key, self_paced=False):
    return CertificateGenerationConfiguration.current().enabled and \
           (self_paced or CertificateGenerationCourseSetting.is_self_generation_enabled_for_course(course_key))
