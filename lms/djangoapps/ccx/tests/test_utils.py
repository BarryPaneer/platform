...
class TestStaffOnCCX(CcxTestCase):
    ...
    @skip("skipped as ccx_customcourseforedx table does not exit in production")
    def test_add_master_course_staff_to_ccx(self):
        ...
        with ccx_course(self.ccx_locator) as course_ccx:
            ...
            self.assertEqual(len(list_staff_master_course), len(list_staff_ccx_course) + 1)
            # self.assertEqual(list_staff_master_course[0].email, list_staff_ccx_course[0].email)

            list_instructor_ccx_course = list_with_level(course_ccx, 'instructor')
            self.assertEqual(len(list_instructor_ccx_course) + 1, len(list_instructor_master_course))
    ...
    def test_add_master_course_staff_to_ccx_display_name(self):
        ...
        self.assertEqual(len(outbox) + 2, len(list_staff_master_course) + len(list_instructor_master_course))
        for email in outbox:
