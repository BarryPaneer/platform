...
def get_course_by_id(course_key, depth=0):
    ...
    if course:
        ...
        if course.display_name != course_overview.display_name:
            course.display_name = course_overview.display_name
            from triboo_analytics.models import ANALYTICS_WORKER_USER, create_analytics_worker
            try:
                analytics_worker = User.objects.get(username=ANALYTICS_WORKER_USER)
            except User.DoesNotExist:
                analytics_worker = create_analytics_worker()

            modulestore().update_item(course, analytics_worker.id)
        return course
    else:
...
def get_courses(user, org=None, filter_=None):
    ...
    """
    # Flag: query for teams courses of user only
    teams_courses_only = filter_.pop('teams_courses_only', False) \
        if filter_ \
        else False
    # Get user id of teams courses
    user = user.id \
        if not isinstance(user, AnonymousUser) and teams_courses_only \
        else None
    # Return all flag
    return_all_flag = True \
        if not filter_ else filter_.pop('all', True)

    courses = branding.get_visible_courses(
        org=org,
        filter_=filter_,
        teams_courses_user=user,
