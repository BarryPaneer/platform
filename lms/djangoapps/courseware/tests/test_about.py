...
class AboutWithInvitationOnly(SharedModuleStoreTestCase):
    ...
    def test_invitation_only_but_allowed(self):
        ...
        resp = self.client.get(url)
        self.assertEqual(resp.status_code, 200)
        self.assertIn('<a href="#" class="register">Enroll</a>', resp.content)
...
class CourseAboutTestCaseCCX(SharedModuleStoreTestCase, LoginEnrollmentTestCase):
    ...
    @skip("no redirection any more, it will stay on course_about page")
    def test_redirect_to_dashboard_unenrolled_ccx(self):
        ...
        Redirect him to course about page
        """
        ...
        expected = reverse('about_course', args=[ccx_locator])
        self.assertRedirects(response, expected, status_code=200, target_status_code=200)
