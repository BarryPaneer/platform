...
def csv_student_field_validation(first_name, last_name, username, email, password, gender,
                                 year_of_birth, language, country, company, hire_date, row_num,
                                 check_mandantory_fields=True):
    row_errors = []

    if len(first_name) == 0 and check_mandantory_fields:
        row_errors.append({'response': _('Row #{row_num}: A first name is required.').format(row_num=row_num)})

    if len(last_name) == 0 and check_mandantory_fields:
    ...
    if len(password) == 0 and check_mandantory_fields:
        row_errors.append({'response': _('Row #{row_num}: A password is required.').format(row_num=row_num)})

    if gender.lower() not in {'m', 'f', 'o'} and (check_mandantory_fields or len(gender) > 0):
    ...
    if len(language) == 0:
        ...
        released_languages_codes = [released_language.code for released_language in released_languages()]
        if language not in released_languages_codes:
            row_errors.append({
                'response': _('Row #{row_num}: Invalid language, a language code is expected ({languages}).')
                    .format(row_num=row_num, languages=', '.join(map(str, released_languages_codes)))
            })
    ...
    if len(company) == 0 and check_mandantory_fields:
    ...
    if len(hire_date) > 0:
        try:
            ...
            row_errors.append({'response': _('Row #{row_num}: Invalid hire date (expected format: YYYY-MM-DD).').format(
                row_num=row_num)})
...
def _register_and_enroll_students_precheck(request):
    """
    ...
    if 'students_list' in request.FILES:
        ...
        for student in students:
            ...
            try:
                validation_errors, valid_username, valid_email = csv_student_field_validation(first_name, last_name,
...
def _batch_update_student(request):
    """
    ...
    if 'students_list' in request.FILES:
        ...
        for student in students:
            ...
            password = student[LT_CSV['password'] + 2].strip()
            ...
            if not old_username and not old_email:
                row_errors.append({'response': _('Row #{row_num}: An old username or email address is required.').format(row_num=row_num)})
                continue


            if old_username and (len(old_username) == 1 or is_username_retired(old_username)):
                row_errors.append({
                    'response': _('Row #{row_num}: Invalid old username.').format(row_num=row_num),
                })
                continue

            if old_email and is_email_retired(old_email):
                row_errors.append({
            ...
            try:
                validation_errors, valid_username, valid_email = csv_student_field_validation(
                                            ...
                                            password, gender, year_of_birth, language, country, company, hire_date,
                                            row_num, check_mandantory_fields=False)
                ...
                if len(validation_errors) == 0:
                    ...
                    if old_email:
                        try:
                            ...
                            # see if it is an exact match with email and username
                            # if it's not an exact match then just display an error message
                            if len(old_username) >= 2 and user.username != old_username:
                                row_errors.append({
                                    'response': _(
                            ...
                            row_errors.append({
                                'response': _('Row #{row_num}: No account was found with the provided email.')
                                    .format(row_num=row_num)
                            })
                    else:
                        try:
                            ...
                            row_errors.append({
                                'response': _('Row #{row_num}: No account was found with the provided username.')
                                    .format(row_num=row_num)
                            })
                            ...
                            if username_needs_update:
                                updated.append({
                                    'response': _('Row #{row_num}: user {old} (now: {new}) successfully updated.')
                                        .format(row_num=row_num, old=(old_email or old_username), new=new_username)
                                })
                            else:
                                updated.append({
                                    'response': _('Row #{row_num}: user {old} successfully updated.')
                                        .format(row_num=row_num, old=(old_email or old_username))
                                })
...
def send_welcoming_email(request, course_id):
    return _send_welcoming_email(request, course_id)


def _send_welcoming_email(request, course_id=None):
    """
...
def lt_update_profile(profile, first_name, last_name,
    ...
    if first_name and last_name:
        profile.name = last_name + ' ' + first_name
    if gender:
    ...
    if language:
        UserPreference.objects.update_or_create(user=profile.user,
                                                key=LANGUAGE_KEY,
...
class _PartialUnenrollException(Exception):
    """The exception will be raised when we try to unenroll from a course which also belong to Program(s).
        &
        The request user is also enrolled in that Program(s).
    """
    pass


class _InactiveUserException(Exception):
    """
    The exception will be raised when a user is inactive
    """
    pass
...
@cache_control(no_cache=True, no_store=True, must_revalidate=True)
@require_level('staff')
@require_post_params(action="enroll or unenroll")
def students_bulk_enroll(request, course_id):
    """
    Enroll or unenroll students by emails/usernames/user_ids.
    Requires staff access.

    Query Parameters:
    - action in ['enroll', 'unenroll']
    - identifiers is string containing a list of user_ids and/or emails and/or usernames separated by anything split_input_list can handle.
    ...
    for identifier in identifiers:
        ...
        user = None
        email = None
        username_or_email = identifier.get('username') or identifier.get('email') or None
        try:
            # user_id
            ...
            continue
        except _InactiveUserException:
            # Flag as an error if the user is inactive,
            # but continue checking the remaining of the list
            results.append(
                {
                    'identifier': identifier,
                    'inactiveUser': True
        ...
        try:
            ...
            if action == 'enroll':
                ...
                before_enrollment = before['enrollment']
                ...
                if ProgramsApiConfig.is_student_dashboard_enabled():
                    ...
                    if user_enrolled_programs_count > 0:
                        raise _PartialUnenrollException(
                            'Cannot unenroll user: {} form course: {}, because the course is part of a learning path he/she is enrolled in'.format(
                                user.email, str(course_id)
                            )
                        )
            ...
            # Flag as an error if we only unenroll this user from course before Program,
            # but continue checking the remaining of the list
            results.append(
                {
                    'identifier': identifier,
            ...
            # Flag this identifier as an error if invalid, but continue checking
            ...
            if (no_email_address and not email.endswith(no_email_address)) or \
                    (no_email_address is None and email is not None):
                ManualEnrollmentAudit.create_manual_enrollment_audit(
                    request.user, email, state_transition, 'bulk_enroll', enrollment_obj
                )
                results.append({
                    'identifier': identifier,
                    'before': before,
