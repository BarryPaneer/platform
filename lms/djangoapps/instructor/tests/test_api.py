...
class TestInstructorAPIBulkAccountCreationAndEnrollment(SharedModuleStoreTestCase, LoginEnrollmentTestCase):
    ...
    def test_language_bad_format_in_csv(self):
        ...
        self.assertEquals(len(data['general_errors']), 0)
        self.assertEquals(len(data['row_errors']), 1)
        languages = ', '.join(map(str, [released_language.code for released_language in released_languages()]))
        invalid_lang_message = 'Row #1: Invalid language, a language code is expected ({languages}).'.format(
            languages=languages)
        self.assertEquals(data['row_errors'][0]['response'], invalid_lang_message)
        manual_enrollments = ManualEnrollmentAudit.objects.all()
        self.assertEqual(manual_enrollments.count(), 0)
...
class TestInstructorAPIBulkAccountUpdate(SharedModuleStoreTestCase, LoginEnrollmentTestCase):
    ...
    def test_old_username_and_email_missing_in_csv(self):
        csv_content = ("First,Last,test_student_1,test_student_1@example.com,password,o,1988,"
                        "fr,FR,Paris,Site1,Company,123456,2018-11-27,Job,Department,Supervisor,"
                        "group1,comments")
        uploaded_file = SimpleUploadedFile("temp.csv", csv_content)
        ...
        self.assertEquals(data['updated'][0]['response'], "Row #1: user test_student_1@example.com (now: New_test_student_1) successfully updated.")

        csv_content = ("New_test_student_1,,"
                        "New_First,New_Last,New_test_student_2,New_test_student_1@example.com,New_password,f,1978,"
                        "en,US,New_Paris,New_Site,New_Company,98765,2019-11-27,New_Job,New_Department,New_Supervisor,"
                        "New_group,New_comments")
        uploaded_file = SimpleUploadedFile("temp.csv", csv_content)
        response = self.client.post(self.url, {'students_list': uploaded_file})
        ...
        self.assertEquals(len(data['row_errors']), 0)
        self.assertEquals(len(data['updated']), 1)
        self.assertEquals(data['updated'][0]['response'], "Row #1: user New_test_student_1 (now: New_test_student_2) successfully updated.")
        ...
        self.assertEquals(len(data['general_errors']), 0)
        self.assertEquals(len(data['row_errors']), 1)
        self.assertEquals(data['row_errors'][0]['response'], "Row #1: An old username or email address is required.")
    ...
    def test_old_username_or_email_not_found(self):
        ...
        self.assertEquals(data['row_errors'][1]['response'], "Row #2: No account was found with the provided email.")
        self.assertEquals(data['row_errors'][2]['response'], "Row #3: No account was found with the provided username.")
        self.assertEquals(len(data['updated']), 0)
