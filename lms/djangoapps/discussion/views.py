...
def get_threads(request, course, user_info, discussion_id=None, per_page=THREADS_PER_PAGE):
    ...
    id_map = get_cached_discussion_id_map(
        course,
        [thread['commentable_id'] for thread in threads],
        request.user
    )
    gated_content = []
    if not has_discussion_privileges(request.user, course.id):
        gated_content = gating_api.get_gated_content(course, request.user)
    filtered_threads = []
    for thread in threads:
        ...
        if commentable_id in id_map:
            location = id_map[commentable_id]["location"]
            location = modulestore().get_item(location)
            try:
                location = location.get_parent().get_parent().location
            except AttributeError:
                location = location.get_parent().location
            if text_type(location) not in gated_content:
