...
from lms.djangoapps.courseware.views import OverviewPage as CoursesProgramsOverviewPage
from lms.djangoapps.courseware.views import ProgramsPage
from lms.djangoapps.my_training.views import TrainingOverviewPage
from lms.djangoapps.my_training.views import MyTrainingProgramsPage
...
from openedx.features.centrical_support.views import triboo_centrical_catalog
...
urlpatterns = [
    ...
    url(r'my_training_overview/', TrainingOverviewPage.as_view(), name='my_training_overview'),
    url(r'my_training_programs/', MyTrainingProgramsPage.as_view(), name='my_training_programs'),
    ...
    # Programs Enrollments API RESTful endpoints
    url(r'api/program_enrollments/', include('program_enrollments.rest_api.urls')),
    ...
    # Extended API
    url(r'^api/extended/', include('extended_api.urls', namespace='extended_api')),

    # Triboo Centrical Platform
    url(r'^triboo_centrical/$', triboo_centrical_catalog, name=r'triboo_centrical_platform'),

    # Blocks Batch Completionn
    url(r'{}/batch_completion/$'.format(settings.COURSE_ID_PATTERN), courseware_views.batch_completion,
...
urlpatterns += [
    ...
    url(r'^programs/$', ProgramsPage.as_view(), name='programs'),
    url(r'^courses_overview/$', CoursesProgramsOverviewPage.as_view(), name='courses_overview'),
    ...
    url(r'^courses/linkedin_catalog$', external_catalog_views.linkedin_catalog, name='linkedin_catalog'),
    url(r'^courses/udemy_catalog$', external_catalog_views.udemy_catalog, name='udemy_catalog'),
    url(r'^courses/founderz_catalog$', external_catalog_views.founderz_catalog, name='founderz_catalog'),
    url(r'^courses/siemens_catalog$', external_catalog_views.siemens_catalog, name='siemens_catalog'),
    ...
    url(
        r'^programs/{}/about2$'.format(
            r'(?P<program_uuid>[0-9a-f-]+)',
        ),
        courseware_views.program_marketing2,
        name='program_marketing_view2',
    ),
    url(
...
if settings.FEATURES.get('ENABLE_DISCUSSION_SERVICE'):
    urlpatterns += [
        ...
        url(
            r'^ilt-attendance-sheet/{course_key}/{usage_key}/(?P<sess_key>[0-9]+)/$'.format(
                course_key=settings.COURSE_ID_PATTERN,
                usage_key=settings.USAGE_ID_PATTERN,
            ),
            ilt_attendance_sheet,
            name='ilt_attendance_sheet'
        ),
    ]
...
urlpatterns += [
    ...
    url(r'^update_instructor_task$', certificates_views.update_instructor_task, name='update_instructor_task'),
