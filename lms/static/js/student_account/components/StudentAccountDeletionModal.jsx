...
class StudentAccountDeletionConfirmationModal extends React.Component {
  ...
  renderConfirmationModal() {
    ...
    const notationText = StringUtils.interpolate(
        gettext('If you proceed, you will be unable to use this account to take courses on the {platformName}.'),
        {
            platformName: this.props.platformName
        }
    );
    const deletionText = StringUtils.interpolate(
        gettext('You have selected “Delete my account.” Deletion of your account and personal data is permanent and cannot be undone. You may also lose access to verified certificates. {platformName} will not be able to recover your account or the data that is deleted.'),
        {
            platformName: this.props.platformName
        }
    );
              ...
              <StatusAlert
                dialog={(
                  <div className="modal-alert">
                    ...
                    <div className="alert-content">
                      <h3 className="alert-title">{ deletionText }</h3>
                      <p>{ notationText }</p>
                    </div>
                  </div>
                )}
