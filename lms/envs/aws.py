...
GRADES_QUEUE = 'edx.{0}core.grade'.format(QUEUE_VARIANT)
PROGRESS_QUEUE = 'edx.{0}core.progress'.format(QUEUE_VARIANT)
LEADERBOARD_QUEUE = 'edx.{0}core.leaderboard'.format(QUEUE_VARIANT)
...
LEARNER_NO_EMAIL = ENV_TOKENS.get('LEARNER_NO_EMAIL', LEARNER_NO_EMAIL)
DEFAULT_FROM_EMAIL_ALIAS = ENV_TOKENS.get('DEFAULT_FROM_EMAIL_ALIAS', DEFAULT_FROM_EMAIL_ALIAS)
...
# Currency
PAID_COURSE_REGISTRATION_CURRENCY = ENV_TOKENS.get(
...
EDX_OBJECTS_MONGODB_SETTINGS = AUTH_TOKENS.get('EDX_OBJECTS_MONGODB_SETTINGS', {})
...
MULTI_SITE_HOSTNAME_MODULESTORE_MAPPINGS = ENV_TOKENS.get('MULTI_SITE_HOSTNAME_MODULESTORE_MAPPINGS', {})
if 'SITE_PREVIEW_LMS_LIST' in FEATURES and FEATURES['SITE_PREVIEW_LMS_LIST'] != []:
    for preview_domain_name in FEATURES['SITE_PREVIEW_LMS_LIST']:
        MULTI_SITE_HOSTNAME_MODULESTORE_MAPPINGS.update({preview_domain_name: 'draft-preferred'})
...
HEARTBEAT_ELASTICSEARCH_TIMEOUT = ENV_TOKENS.get('HEARTBEAT_ELASTICSEARCH_TIMEOUT', HEARTBEAT_ELASTICSEARCH_TIMEOUT)
...
# Triboo analytics reports
TRIBOO_ANALYTICS_REPORTS = ENV_TOKENS.get("TRIBOO_ANALYTICS_REPORTS", TRIBOO_ANALYTICS_REPORTS)
...
if FEATURES.get('ENABLE_THIRD_PARTY_AUTH'):
    ...
    SOCIAL_AUTH_OKTA_OAUTH2_KEY = AUTH_TOKENS.get('SOCIAL_AUTH_OKTA_OAUTH2_KEY', '')
    SOCIAL_AUTH_OKTA_OAUTH2_SECRET = AUTH_TOKENS.get('SOCIAL_AUTH_OKTA_OAUTH2_SECRET', '')
    SOCIAL_AUTH_OKTA_OAUTH2_API_URL = AUTH_TOKENS.get('SOCIAL_AUTH_OKTA_OAUTH2_API_URL', '')
...
PROGRAM_DISCOVERY_FILTERS = ['language', 'start', 'vendor']
...
if FEATURES.get('ENABLE_SAML_IDP'):
    ...
    SAML_IDP_CONFIG = {
        ...
        "attribute_map_dir": os.path.abspath('common/djangoapps/third_party_auth/attributemaps'),
        'metadata': AUTH_TOKENS.get('SAML_IDP_METADATA_URLS'),
        # Signing
